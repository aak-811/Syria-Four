...
function StatCard({ icon, value, label, suffix = "", delay = 0, color = "#00E5FF" }: {
  icon: React.ReactNode; value: number; label: string; suffix?: string; delay?: number; color?: string;
}) {
  return (
    <div className="fade-in relative group" style={{ animationDelay: `${delay}s` }}>
...
export default function PublicHomePage() {
  ...
  useEffect(() => {
    const p1 = api.getMembers().catch(() => []);
    const p2 = api.getTournaments().catch(() => fallbackTournaments);
    const p3 = api.getEvents().catch(() => []);
    const p4 = api.getGallery().catch(() => fallbackGallery);
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="text-center py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="w-32 h-32 rounded-2xl mx-auto mb-6 shadow-[0_0_60px_rgba(0,229,255,0.3)] relative overflow-hidden bg-gradient-to-br from-[#00E5FF] to-[#8B5CF6] flex items-center justify-center">
          <span className="text-white font-black text-4xl tracking-tighter">S4</span>
        </div>

        <h1 className="text-4xl md:text-6xl font-black mb-3 bg-gradient-to-l from-[#00E5FF] via-[#8B5CF6] to-[#FFD700] bg-clip-text text-transparent">
          SYRIA FOUR
        </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
        <StatCard icon={<Users size={28} />} value={120} label="Members" suffix="+" delay={0} color="#00E5FF" />
        <StatCard icon={<Swords size={28} />} value={15} label="Tournaments" suffix="+" delay={0.1} color="#8B5CF6" />
        <StatCard icon={<Trophy size={28} />} value={380} label="Wins" suffix="+" delay={0.2} color="#FFD700" />
        <StatCard icon={<Eye size={28} />} value={40} label="Views" suffix="K+" delay={0.3} color="#00E676" />
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {members.slice(0, 6).map((m, i) => (
            <div key={m.id || i} className="fade-in" style={{ animationDelay: `${i * 0.05}s` }}>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayLeaders.slice(0, 6).map((l, i) => (
              <div key={l.id || i} className="fade-in" style={{ animationDelay: `${i * 0.08}s` }}>
                <GlassCard className="text-center py-6 relative overflow-hidden group">
        ...
        <div>
          ...
          <div className="grid gap-4">
            {tournaments.slice(0, 4).map((t, i) => (
              <div key={t.id || i} className="fade-in" style={{ animationDelay: `${i * 0.08}s` }}>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {gallery.slice(0, 4).map((img, i) => (
              <div key={i} className="fade-in rounded-[14px] overflow-hidden aspect-square group cursor-pointer relative" style={{ animationDelay: `${i * 0.1}s` }}>
                <img src={img.src} alt={img.label || ""}
                  className="w-full h-full object-cover transition-all duration-500 group-hover:scale-110"
                  onError={(e) => {
                    const target = e.target as HTMLImageElement;
                    target.style.display = "none";
                    target.parentElement!.classList.add("bg-[rgba(255,255,255,0.06)]", "flex", "items-center", "justify-center");
                    target.parentElement!.innerHTML = `<span class="text-[#6B7280] text-sm">${img.label || "صورة"}</span>`;
                  }}
                />
