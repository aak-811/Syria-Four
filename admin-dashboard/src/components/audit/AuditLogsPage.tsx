    ...
    try {
      ...
      (members || []).forEach((m: any) => {
        if (m.joinDate) {
          entries.push({
            id: `join-${m._id}`,
            type: "member_joined",
            description: `${m.name} joined the clan`,
            date: m.joinDate,
            details: `Role: ${m.role || "member"} | Country: ${m.country || "—"}`,
          });
        }
        if (m.role && m.role !== "member") {
          entries.push({
            id: `role-${m._id}`,
            type: "role_change",
            description: `${m.name} assigned as ${m.role}`,
            date: m.joinDate || new Date().toISOString(),
            details: `Level: ${m.level || "—"} | Game ID: ${m.uid || "—"}`,
          });
        }
      });
      ...
      (tournaments || []).forEach((t: any) => {
        entries.push({
          ...
          description: `Tournament created: ${t.title}`,
          date: t.createdAt || new Date().toISOString(),
          details: `${(t.participants || []).length} participants | ${t.game || "—"}`,
        });
      });
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-[#9CA3AF] text-sm mt-4">Loading audit logs...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-[#9CA3AF] text-sm mt-1">Track recent activity and changes</p>
      </motion.div>
      ...
      <GlassCard className="bg-[rgba(255,193,7,0.05)] border border-[rgba(255,193,7,0.15)]">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-[#FFC107]">Limited Audit Data</p>
            <p className="text-xs text-[#9CA3AF] mt-1">
              Full audit logging requires the auth system and dedicated audit endpoints.
              Showing the last 20 activities reconstructed from available member and tournament data.
            </p>
          </div>
        </div>
      </GlassCard>
      ...
      {activities.length === 0 ? (
        <GlassCard>
          <div className="text-center py-10">
            ...
            <p className="text-[#9CA3AF] font-semibold">No activity recorded</p>
            <p className="text-[#6B7280] text-sm mt-1">Activity will appear as members and tournaments are added</p>
          </div>
        </GlassCard>
      ) : (
