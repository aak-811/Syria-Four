...
export default function ShopPage() {
  ...
  useEffect(() => {
    api.getOrders().then(data => setItems(data.length > 0 ? data : fallback))
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <GlassCard className="p-12 text-center">
            <ShoppingBag size={48} className="mx-auto text-[#6B7280] mb-4" />
            <p className="text-[#9CA3AF]">لا توجد منتجات متاحة حاليًا</p>
          </GlassCard>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((item, i) => (
              <div key={item.id || i} className="animate-fade-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
                <GlassCard>
