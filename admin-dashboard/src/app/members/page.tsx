...
const fallback = [
  ...
  { id: "4", name: "Elite1", gameId: "Elite-001", role: "elite", level: 70, wins: 95, tournaments: 12, country: "SY" },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map((m, i) => (
              <div key={m.id} className="animate-fade-slide-up" style={{ animationDelay: `${i * 0.04}s` }}>
                <GlassCard hover onClick={() => setSelected(m)} className="text-center py-6 relative overflow-hidden group cursor-pointer">
          ...
          {selected && (
            <div className="text-center">
              ...
              <div className="flex justify-center gap-2 mt-3 flex-wrap">
                {selected.role && <Badge variant={roleColors[selected.role] || "default"}>{selected.role === "leader" ? "قائد" : selected.role === "vice" ? "نائب" : selected.role === "chief" ? "زعيم" : selected.role === "elite" ? "نخبة" : "عضو"}</Badge>}
